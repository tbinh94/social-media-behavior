...
import { ApiChartDataPoint } from '../types';
import ChartCard from './ChartCard';

ChartJS.register(ArcElement, Tooltip, Legend);

const PlatformPopularityChart: React.FC = () => {
    const [chartData, setChartData] = useState<ChartData<'pie'> | null>(null);
    const [error, setError] = useState<string>('');

    useEffect(() => {
        axios.get<ApiChartDataPoint[]>('http://127.0.0.1:8000/api/platform-popularity')
            .then(response => {
            ...
            .catch(err => {
                setError("Không thể tải dữ liệu nền tảng.");
                console.error(err);
            });
    }, []);

    return (
        <ChartCard title="Nền tảng được sử dụng nhiều nhất">
...
export default PlatformPopularityChart;```

**File: `src/components/OccupationChart.tsx`**
```typescript
import React, { useState, useEffect } from 'react';
import axios from 'axios';
